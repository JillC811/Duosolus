...
public class Cyclope2Movement : MonoBehaviour, InterfaceUndo
{
    ...
    private const string Cyclope2_DEATH_SFX_FILEPATH = "SFX/Hit7";

    // Start is called before the first frame update
    void Start()
    {
        GameStateManager.Instance.cyclope2Active = true;
    ...
    void Update()
    {
        ...
        if (GameStateManager.Instance.Cyclope2Moving > 0 || Mathf.Abs(Input.GetAxisRaw("Vertical")) == 1f || Mathf.Abs(Input.GetAxisRaw("Horizontal")) == 1f) {
        ...
        if(transform.position == MovePoint.position && GameStateManager.Instance.Cyclope2Moving > 0)
        {
            GameStateManager.Instance.Cyclope2Moving = 0;
            ...
            if(Physics2D.OverlapPoint(new Vector2(transform.position.x, transform.position.y), Death))
            {
                ...
                // SFX
                AudioClip clip = Resources.Load<AudioClip>(Cyclope2_DEATH_SFX_FILEPATH);
        ...
        // If Cyclope2 dead, don't do anything
        ...
        if(!GameStateManager.Instance.PlayerMoving)
        {
            if(Mathf.Abs(Input.GetAxisRaw("Horizontal")) == 1f)
            {
                ...
                {
                    GameStateManager.Instance.Cyclope2Moving += 1;
