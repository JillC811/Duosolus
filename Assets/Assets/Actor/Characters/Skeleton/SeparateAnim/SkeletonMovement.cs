...
public class SkeletonMovement : MonoBehaviour, InterfaceUndo
{
    ...
    private const string Skeleton_DEATH_SFX_FILEPATH = "SFX/Hit7";

    // Start is called before the first frame update
    void Start()
    {
        GameStateManager.Instance.skeletonActive = true;
    ...
    void Update()
    {
        ...
        if (GameStateManager.Instance.SkeletonMoving > 0 || Mathf.Abs(Input.GetAxisRaw("Vertical")) == 1f || Mathf.Abs(Input.GetAxisRaw("Horizontal")) == 1f) {
        ...
        if(transform.position == MovePoint.position && GameStateManager.Instance.SkeletonMoving > 0)
        {
            GameStateManager.Instance.SkeletonMoving = 0;
            ...
            if(Physics2D.OverlapPoint(new Vector2(transform.position.x, transform.position.y), Death))
            {
                ...
                // SFX
                AudioClip clip = Resources.Load<AudioClip>(Skeleton_DEATH_SFX_FILEPATH);
        ...
        // If Skeleton dead, don't do anything
        ...
        if(!GameStateManager.Instance.PlayerMoving)
        {
            if(Mathf.Abs(Input.GetAxisRaw("Horizontal")) == 1f)
            {
                ...
                {
                    GameStateManager.Instance.SkeletonMoving += 1;
